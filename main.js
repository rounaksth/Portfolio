...
function initTheme() {
  ...
  // 1. Read stored preference; default strictly to "light"
  const savedTheme = localStorage.getItem("theme");
  const currentTheme = savedTheme || "light";

  // 2. Set theme on document root WITHOUT force-writing to localStorage on load
  document.documentElement.setAttribute("data-theme", currentTheme);
  toggleBtn.innerHTML = currentTheme === "dark" ? sunIcon : moonIcon;

  // 3. Update theme ONLY on explicit user click
